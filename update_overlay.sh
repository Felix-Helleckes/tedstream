...
# Loop 1: Data Update (Runs in background, every 2s)
(
  last_news_update=0
  ...
  while true; do
    ...
    if [ $((now - last_news_update)) -ge 180 ]; then
      (
        ...
        [ -z "$M_LINE" ] && M_LINE="Market Monitoring Active   ***   24/7 Trading Stream"
        echo "$M_LINE" | sed 's/%/\\%/g' > "$TEMP_DIR/news_marquee.txt"
      ) &
