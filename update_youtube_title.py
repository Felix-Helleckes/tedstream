...
if not all([client_id, client_secret, refresh_token]):
    print(f"[title-updater] Missing required credentials in .env. Skipping (would set day: {day_number})")
    sys.exit(0)
...
try:
    ...
    # Get active broadcast (assuming there is exactly one for the 24/7 stream)
    resp = http_json(
        "GET",
        f"{base}/liveBroadcasts?part=snippet&broadcastStatus=active",
        headers=headers,
    )
    if not resp.get("items"):
        print("[title-updater] No active broadcast found. Skipping.")
        sys.exit(0)
    broadcast = resp["items"][0]
    broadcast_id = broadcast["id"]
    current_snippet = broadcast["snippet"]
    current_title = current_snippet["title"]
    print(f"[title-updater] Current title: {current_title}")
    ...
    # Update the broadcast with the modified snippet
    payload = json.dumps({
        "id": broadcast_id,
    ...
    resp = http_json(
        "PUT",
        f"{base}/liveBroadcasts?part=snippet",
    ...
    print(f"[title-updater] Title updated to: \"{new_title}\" for broadcast {broadcast_id}")
